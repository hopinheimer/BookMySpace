...
public class BookingServiceImpl implements BookingService {
    ...
    public BookingResponseModel bookSeat(BookRequestModel request) throws BookSpaceException {
        ...
        Optional<MembershipEntity> membershipOptional = membershipRepository.getMembershipBydate(request.getStartDate(), request.getEndDate(), request.getProductId());
        ...
        InvoiceDto invoice = getInvoice(product);
    ...
    private InvoiceDto getInvoice(ProductEntity product) throws BookSpaceException {
        InvoiceDto invoice = new InvoiceDto();
        invoice.setServiceTax(product.getPrice() * 0.02);
        invoice.setTax(product.getPrice() * 0.18);
        invoice.setTotalAmount(product.getPrice());
        invoice.setTotalAmount(product.getPrice()+invoice.getTax()+ invoice.getServiceTax());
        return invoice;
    }
